...
public class SlowFryRecipe extends BaseRecipe<FryerCraftContainer> implements SingleInputRecipe {
    ...
    public boolean matches(FryerCraftContainer pContainer, Level pLevel) {
        ...
        if (cookingTime < SLOW_COOKING_TIME || cookingTime > FAIL_COOKING_TIME)
            return false;
    ...
    public static class Serializer extends BaseRecipeSerializer<SlowFryRecipe> {
        @Override
        public SlowFryRecipe fromJson(ResourceLocation pRecipeId, JsonObject pSerializedRecipe) {
            return new SlowFryRecipe(
                    pRecipeId,
