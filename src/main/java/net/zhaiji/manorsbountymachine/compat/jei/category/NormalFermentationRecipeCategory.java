...
public class NormalFermentationRecipeCategory implements IRecipeCategory<NormalFermentationRecipe> {
    ...
    @Override
    public RecipeType<NormalFermentationRecipe> getRecipeType() {
        return ManorsBountyMachineJeiPlugin.NORMAL_FERMENTATION;
    }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, NormalFermentationRecipe recipe, IFocusGroup focuses) {
        if (recipe.hasBottle()) {
    ...
    @Override
    public void draw(NormalFermentationRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, recipeSlotsView, guiGraphics, mouseX, mouseY);
        this.guiHelper.createDrawable(NORMAL_FERMENTATION_RECIPE_BACKGROUND, 0, 0, this.getWidth(), this.getHeight()).draw(guiGraphics);
