...
public class DimFermentationRecipeCategory implements IRecipeCategory<DimFermentationRecipe> {
    ...
    @Override
    public RecipeType<DimFermentationRecipe> getRecipeType() {
        return ManorsBountyMachineJeiPlugin.DIM_FERMENTATION;
    }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, DimFermentationRecipe recipe, IFocusGroup focuses) {
        if (recipe.hasBottle()) {
    ...
    @Override
    public void draw(DimFermentationRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, recipeSlotsView, guiGraphics, mouseX, mouseY);
        this.guiHelper.createDrawable(DIM_FERMENTATION_RECIPE_BACKGROUND, 0, 0, this.getWidth(), this.getHeight()).draw(guiGraphics);
